#!/usr/bin/env python3
"""
Kaggle Kernel Script - Complete Evaluation in One File (FIXED VERSION)
Ready to copy-paste into Kaggle notebooks
"""
...
def evaluate_summac_batch(model, documents, summaries, batch_size=16):
    """Evaluate SummaC-ZS scores (no truncation)."""
    scores = []
...
def evaluate_unieval_batch(evaluator, output_list, dimensions=['fluency', 'naturalness', 'understandability'], batch_size=32):
    """Evaluate UniEval scores (no truncation) - FIXED VERSION."""
    ...
    for i in tqdm(range(0, len(output_list), batch_size), desc="UniEval"):
        ...
        try:
            ...
            # FIXED: Prepare data properly for UniEval
            data = convert_to_json(output_list=filtered_out)
            
            # FIXED: Evaluate each dimension separately to avoid format issues
            for dim in dimensions:
                try:
...
if None in datasets.values():
    ...
    # Create detailed results DataFrame - FIXED KEY NAMES
    results_data = []
